/*
  # Randevu durum değişikliği trigger güncellemesi
  
  1. Değişiklikler
    - Trigger fonksiyonu güncellendi
    - Durum değişikliği kontrolü eklendi
    - Hata yakalama ve loglama iyileştirildi
    
  2. Güvenlik
    - RLS politikaları korundu
    - Kullanıcı doğrulama kontrolleri eklendi
*/
...
DECLARE
  v_user_id uuid;
  v_log_id uuid;
  v_existing_approved_count integer;
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_existing_approved_count
  FROM approved_appointments
  WHERE appointment_id = NEW.id;

  SELECT COUNT(*) INTO v_existing_cancelled_count
  FROM cancelled_appointments
  WHERE appointment_id = NEW.id;

  -- Eğer randevu zaten onaylanmış veya iptal edilmişse işlemi engelle
  IF v_existing_approved_count > 0 OR v_existing_cancelled_count > 0 THEN
    INSERT INTO debug_logs (function_name, message, details)
    VALUES ('handle_appointment_status_change', 'Error: Appointment already processed', jsonb_build_object(
      'appointment_id', NEW.id,
      'approved_count', v_existing_approved_count,
      'cancelled_count', v_existing_cancelled_count
    ));
    RAISE EXCEPTION 'Bu randevu için zaten bir işlem yapılmış';
  END IF;
