/*
  # Update follow-up appointment creation to include previous notes
  
  1. Changes
    - Update create_follow_up_appointment function to include previous meeting notes
    - Copy meeting_note from original appointment to new follow-up appointment
    
  2. Security
    - Maintain existing function security
*/

-- Update the follow-up appointment creation function to include previous notes
...
BEGIN
  -- Orijinal randevu bilgilerini al (meeting_note dahil)
  SELECT 
    lecturer_id, student_id, student_name, student_email,
    student_faculty, student_department, student_year, meeting_note
  INTO 
    v_lecturer_id, v_student_id, v_student_name, v_student_email,
    v_student_faculty, v_student_department, v_student_year, v_meeting_note
  FROM appointments
  ...
  -- Yeni randevu oluştur (önceki notla birlikte)
  ...
  VALUES (
    ...
    v_student_year,
    'Takip Toplantısı',
    'Akademisyen tarafından yeni bir toplantı oluşturulmuştur',
    v_meeting_note, -- Önceki randevunun notunu kopyala
    CURRENT_TIMESTAMP,
    auth.uid()
  )
